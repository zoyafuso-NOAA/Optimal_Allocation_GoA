...
for (irow in 1:nrow(scen)) {
  ...
  frame[, c(paste0("Y", 1:ns_opt), paste0("Y", 1:ns_opt, "_SQ_SUM") )] <-
    cbind(matrix(data = apply(X = dens_vals[, , , paste("Type", scen$type[irow])],
                              MARGIN = c(1, 2), 
                              FUN = sum),
                 ncol = ns_opt,
                 dimnames = list(NULL, paste0("Y", 1:ns_opt))),
          
          matrix(data = apply(X = dens_vals[, , , paste("Type", scen$type[irow])],
                              MARGIN = c(1, 2), 
  ...
  temp_dir = paste0(github_dir, "/results/sensitivity_analysis/", 
                    "Type", scen$type[irow], "_stratavars", scen$strata_vars[irow],
