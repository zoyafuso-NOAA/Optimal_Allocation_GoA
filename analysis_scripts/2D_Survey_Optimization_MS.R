...
for (iscen in c(10)){
  ...
  for (ispp in common_names_opt){
    load( paste0(github_dir, "/results/scenario_", scenarios$scen_name[iscen], "/Single_Species_Optimization/", ispp, "/result_list.RData") )
  ...
  for(temp_strata in no_strata) {
    ...
    srs_n <- as.numeric(280* table(temp_frame$domainvalue) / n_cells)
    srs_var <- as.matrix(srs_stats[, paste0("S", 1:ns_opt)])^2
    ...
    ## Save Output
    # CV_constraints <- expected_CV(strata = temp_stratif)
