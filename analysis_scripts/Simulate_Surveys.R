    ...
    for (ispp in common_names_all) {
      ...
      for (iter in 1:n_iters) {
        ...
        for (iboat in 1:n_boats) {
          ...
          if(idata ==  "sim_dens") {
            temp_sim_data <- sim_data[, , iter]
            cpue <- sweep(x = temp_sim_data,
                          MARGIN = 1,
        ...
        if(iter%%50 == 0) {
          ...
          print(paste0("Finished: Iteration ", iter, ", ", 
                       ispp, ", ", 
                       isurvey, " scenario, ", 
                       scen$strata[irow], " strata, boat ", iboat))
        }
      }
    }
