################################################
## Spatial Only Survey Optimization
################################################
rm(list = ls())
...
###############################
library(sp); library(RColorBrewer); library(raster); library(SamplingStrata)
...
for(istrata in nstrata[4]){
  ...
  while(current_n > 280){
    ...
    save(list = c('res_df', 'strata_list', 'settings'),
         file = paste0(github_dir, 'Spatial_Optimization/optimization_',
