...
## Result Objects
###########################
sim_mean <- sim_cv <- array(dim = c(NTime, ns, nboats, Nstrata, Niters), 
...
#                'sim_mean', 'sim_cv')){
#   assign(value = get(paste0('STRS_', ivar)), x = ivar)
# }
...
#######################
for(ivar in  c('rrmse_cv_array', 'true_cv_array', 
               'sim_mean', 'sim_cv', "rel_bias_cv", 'rel_bias_est')){
...
save(file=paste0(github_dir,
     ...
     list = c(paste0('STRS_', c('rrmse_cv_array', 
                                'true_cv_array', 'sim_mean', 'sim_cv', 
                                "rel_bias_cv", 'rel_bias_est')),
              'Niters'))
