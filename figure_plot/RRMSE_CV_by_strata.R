...
for(itype in c('Spatiotemporal', 'Spatiotemporal_Flexible')){
   ...
   png(file = paste0(PP_dir, 'RRMSE_CV_', itype, '.png'),
   ...
   for(ispp in 1:ns){
      max_val = max(0.11, 1.1*max(STRS_rrmse_cv_array[,ispp,,], na.rm = T))
      ...
      if(max_val < 0.5) axis(side = 2, las = 1, at = seq(0.1,1,0.1) )
      if(max_val > 0.5) axis(side = 2, las = 1, at = seq(0.25,1,0.25) )
      ...
      for(istrata in stratas ){
         boxplot(STRS_rrmse_cv_array[,ispp,istrata,], add = T, axes = F, pch = 16,
                 cex = 0.75, 
