...
{png(file = paste0(PP_dir, 'Bias_Spatiotemporal.png'),
  ...
  for(itype in c('Est', 'CV')){
    for(isample in 1:3){
      ...
      plot(1, type = 'n', ann = F, axes = F, 
           ylim = c(-1.5,1.5)*max(max(bias_box),50), xlim = c(0,1))
      box()
      ...
      lines(y = bias_box['2.5%',], x = seq(0,1,length = 11), lty = 'dotted')
      lines(y = bias_box['97.5%',], x = seq(0,1,length = 11), lty = 'dotted')
    }
