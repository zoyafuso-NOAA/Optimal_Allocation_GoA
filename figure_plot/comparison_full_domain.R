...
##################################################
{
  png(filename = paste0(result_dir, "/Comparison_full_domain.png"),
      width = 190,
      height = 220,
      units = "mm",
      res = 500)
  ...
  for (isample in 2) {
    for (ispp_ in 1:ns) {
      ...
      #Relative Bias
      ylim_ <- max(abs(
        apply(X = cbind(STRS_rel_bias_est[1, , ispp_, isample, ], 
                        Current_rel_bias_est[1, , ispp_, isample, ]),
              MARGIN = 1,
              FUN = quantile,
              probs = c(0.025, 0.975),
              na.rm = T)
      ...
      plot_percentiles(values = Current_rel_bias_est[1, , ispp_, isample,],
                       xs = 13:23, 
                       ispp = ispp_,
                       plot_years = F,
                       inner_color = "tomato2",
      ...
      ylim_ <- max(abs(cbind(STRS_true_cv_array[1, , ispp_, isample],
                             Current_true_cv_array[1, , ispp_, isample])))
      
      boxplot(cbind(STRS_true_cv_array[1, , ispp_, isample],
                    Current_true_cv_array[1, , ispp_, isample]),
              ylim = c(0, 1.25 * ylim_),
              las = 1,
              axes = F,
      ...
      #RRMSE of CV
      ylim_ <- max(abs(cbind(STRS_rrmse_cv_array[1, , ispp_, isample],
                             Current_rrmse_cv_array[1, , ispp_, isample])))
      
      boxplot(cbind(STRS_rrmse_cv_array[1, , ispp_, isample],
                    Current_rrmse_cv_array[1, , ispp_, isample]),
              ylim = c(0, 1.25 * ylim_),
              las = 1,
              names = NA,
