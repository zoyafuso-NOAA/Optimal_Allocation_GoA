...
#######################################
{
    ...
    layout(mat = matrix(1:18, ncol = 3, byrow = T), heights = c(1,1,1,1,1,1.5))

    for(ispp in 1:ns){     #For Each species
        ...
        #Plot density raster
        image(goa_ras, col = c('white', brewer.pal(n = 9, name = 'Blues')),
        ...
        legend(x = bbox_[1] + xrange*0.575,
               ...
               legend = '200 m\nisopleth', 
               cex = 0.85, lty = 1, bty = 'n', lwd = 2)
        
        #Overlay Land
        plot(AK, add = T, col = 'tan', border = F)
        
        #Overlay 200 m isobar
        plot(bathy_iso_200, add = T, lty = 1, col = 'black')
        ...
        plotrix::color.legend(
            ...
            rect.col = c('white', brewer.pal(n = 9, name = 'Blues')), 
            cex = 0.5, align = 'rb' )
        plotrix::color.legend(
            xl = bbox_[1] + xrange*0.4,
            xr = bbox_[1] + xrange*1,
    ...
    mapPlot(coastlineCut(coastlineWorldFine, -135),
            longitudelim=c(-180, -70), latitudelim=c(20, 65),
            proj="+proj=stere +lat_0=30 +lon_0=-130", col='tan',
            border = 'black' )
    rect(xleft = -2800000, xright = 200000,
         ybottom = 2600000, ytop = 3600000,
         lwd = 3)
