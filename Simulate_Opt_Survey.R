...
which_opt = c('spatiotemporal', 'spatiotemporal_wo_RFs')[2]
load(paste0(github_dir, 'Spatiotemporal_Optimization_Scheme',
            c('spatiotemporal'=2, 'spatiotemporal_wo_RFs'=3)[which_opt] ,
...
for(isample in 1:3) {
  ...
  for(iyear in 1:NTime){
    for(iter in 1:Niters){
      ...
      sim_mean[paste0('Year_',iyear),,isample,iter] = SRS_mean
      sim_cv[paste0('Year_',iyear),,isample,iter] = strata_cv
...
for(iyear in 1:NTime){
  ...
  for(isample in 1:3){
    for(spp in sci_names){
      ...
      rrmse_est_array[paste0('Year_', iyear), spp,isample] = 
...
for(ivar in  c('rrmse_est_array', 'rrmse_cv_array', 'true_cv_array', 
               'sim_mean', 'sim_cv')){
  assign(x=paste0('STRS', c('spatiotemporal'='', 
                            'spatiotemporal_wo_RFs'=2)[which_opt],
                  '_', ivar), value = get(ivar))
}

save(file=paste0(github_dir, 'Spatiotemporal_Optimization_Scheme',
                 c('spatiotemporal'=2, 'spatiotemporal_wo_RFs'=3)[which_opt], 
                 '/STRS_Sim_Res_', which_opt, '.RData'),
     list = c(paste0('STRS', 
                     c('spatiotemporal'='', 
                       'spatiotemporal_wo_RFs'=2)[which_opt],'_', 
                     c('rrmse_est_array', 'rrmse_cv_array', 
